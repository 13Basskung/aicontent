...
import { CheckCircle, XCircle, ExternalLink, Clock, Loader2, TrendingUp, TrendingDown, Users, Wallet, CreditCard, ArrowUpRight, ArrowDownRight, Edit3, DollarSign, BarChart3, Building2, Database, Trash2, RefreshCw, HardDrive, Crown, Star, Sliders, Youtube, Facebook, Instagram, Music2, Save, Eye, EyeOff, Copy } from 'lucide-react';
...
const Admin = () => {
    ...
    const tabs = [
        ...
        { id: 'appSettings', label: 'App Settings', icon: Sliders }
    ];
                            ...
                            <div className="space-y-3 max-h-96 overflow-y-auto custom-scrollbar pr-2">
                                {filteredActivities.map((log, index) => {
                                    ...
                                    const getSubscriptionLabel = () => {
                                        ...
                                        const paidVIPPrice = (log.breakdown?.proPlan ?? 0) > 0;
                                        
                                        if (isNextMonth) {
                                            if (paidVIPPrice && hasAddons) return 'VIP + Add-on เดือนหน้า';
                                            if (paidVIPPrice) return 'VIP Plan เดือนหน้า';
                                            if (hasAddons) return 'Add-on เดือนหน้า';
                                            return 'Subscription เดือนหน้า';
                                        } else {
                                            if (paidVIPPrice && hasAddons) return 'VIP + Add-on เดือนนี้';
                                            if (paidVIPPrice) return 'VIP Plan เดือนนี้';
                                            if (hasAddons) return 'Add-on เดือนนี้';
                                            return 'Subscription เดือนนี้';
                                        }
                                    };
                                    ...
                                    return (
                                        <div 
                                            key={log.id} 
                                            className={`group bg-black/30 rounded-xl p-4 border border-white/5 hover:bg-black/50 hover:border-white/20 transition-all duration-300 ${log.type === 'subscription' ? 'border-l-2 border-l-amber-500' : ''}`}
                                            ...
                                            <div className="flex items-center justify-between gap-4">
                                                ...
                                                {log.type === 'subscription' && (
                                                    <div className="flex items-center gap-2 flex-shrink-0">
                                                        <span className="text-xs text-amber-300 font-medium bg-amber-500/10 px-2 py-1 rounded">
                                                            📦 {getSubscriptionLabel()}
                                                        </span>
                                                        {log.tier && (
                                                            <span className={`text-[10px] px-2 py-1 rounded ${log.tier === 'Premium' ? 'bg-amber-500/20 text-amber-300' : 'bg-purple-500/20 text-purple-300'}`}>
                                                                {log.tier}
                                                            </span>
                                                        )}
                                                        {(log.newExtraProjects ?? log.extraProjects ?? 0) > 0 && (
                                                            <span className="text-xs text-slate-400 bg-slate-500/10 px-2 py-1 rounded">
                                                                +{log.newExtraProjects ?? log.extraProjects} Proj
                                                            </span>
                                                        )}
                                                        {log.billingPeriod && (
                                                            <span className="text-xs text-cyan-300/80 bg-cyan-500/10 px-2 py-1 rounded hidden lg:inline-block">
                                                                📅 {log.billingPeriod.start?.toDate?.()?.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' })} - {log.billingPeriod.end?.toDate?.()?.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' })}
                                                            </span>
                                                        )}
                                                    </div>
                                                )}
                                                ...
                                                <div className="text-right flex-shrink-0 min-w-[100px]">
                                                    ...
                                                    <p className="text-[10px] text-slate-500">โดย <span className="text-purple-300">{formatApprover(log.reviewedByEmail)}</span></p>
                                                </div>
                                            </div>
                                            {log.type === 'subscription' && log.status === 'rejected' && log.rejectReason && (
                                                <p className="text-xs text-red-400 mt-2 pl-14">❌ เหตุผล: {log.rejectReason}</p>
                                            )}
                                        </div>
                                    );
                                })}
...
const AppSettingsTab = ({ selectedPlatform, setSelectedPlatform, appSettings, setAppSettings, savingSettings, setSavingSettings, showSecrets, setShowSecrets, showSuccess, showError }) => {
    const PLATFORMS = [
        {
            id: 'youtube',
            name: 'YouTube',
        ...
        },
        {
            ...
            steps: [
                ...
                'เลื่อนลงมาดูที่ช่อง Valid OAuth Redirect URIs Copy ลิงก์ด้านล่างนี้ไปใส่',
        ...
        },
        {
            ...
            steps: [
                'ใช้ Facebook App เดียวกันกับที่สร้างไว้',
                'ไปที่ Meta Developer → เพิ่ม Instagram Graph API',
                'Instagram Account ต้องเป็น Business/Creator Account',
                'เชื่อมต่อ Instagram กับ Facebook Page ก่อน',
                'Copy App ID และ App Secret จาก Facebook App'
            ]
        },
        {
            id: 'tiktok',
            ...
            fields: [
                { id: 'clientKey', label: 'Client Key', placeholder: 'xxxxxxxxxxxxxxxx', type: 'text' },
            ...
            steps: [
                ...
                'สร้าง App และเลือก Login Kit + Content Posting API',
                'ตั้งค่า Redirect URI: https://aicontents.vip/oauth/callback',
                'Copy Client Key และ Client Secret'
            ]
        }
    ];
    ...
    useEffect(() => {
        const loadSettings = async () => {
            try {
                const settingsRef = doc(db, 'appSettings', selectedPlatform);
                const settingsSnap = await getDoc(settingsRef);
                if (settingsSnap.exists()) {
                    setAppSettings(prev => ({
                        ...prev,
                        [selectedPlatform]: settingsSnap.data()
                    }));
                }
            } catch (error) {
    ...
    const handleSaveSettings = async () => {
        ...
        try {
            const settings = appSettings[selectedPlatform] || {};
            await setDoc(doc(db, 'appSettings', selectedPlatform), {
                ...settings,
                updatedAt: new Date().toISOString()
            });
            showSuccess('✅ บันทึกการตั้งค่าเรียบร้อย', '🎉 สำเร็จ');
        } catch (error) {
            showError('❌ เกิดข้อผิดพลาด: ' + error.message, '🚫 ผิดพลาด');
        } finally {
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                    ...
                    <ol className="space-y-3 mb-6 max-h-[500px] overflow-y-auto pr-2">
                                ...
                                <div className="text-slate-300 text-sm">
                                    ...
                                    {selectedPlatform === 'youtube' && idx === 4 && (
                                        <div className="mt-2 flex items-center gap-2 bg-black/30 rounded-lg p-2">
                                            <code className="text-green-400 text-xs flex-1 break-all">https://aicontents.vip/oauth/callback</code>
                                            <button onClick={() => copyToClipboard('https://aicontents.vip/oauth/callback')} className="p-1.5 hover:bg-white/10 rounded-lg"><Copy size={12} className="text-slate-400" /></button>
                                        </div>
                                    )}
                                    {selectedPlatform === 'facebook' && idx === 5 && (
                                        <div className="mt-2 flex items-center gap-2 bg-black/30 rounded-lg p-2">
                                            <code className="text-green-400 text-xs flex-1 break-all">https://aicontents.vip</code>
                                            <button onClick={() => copyToClipboard('https://aicontents.vip')} className="p-1.5 hover:bg-white/10 rounded-lg"><Copy size={12} className="text-slate-400" /></button>
                                        </div>
                                    )}
                                    ...
                                    {selectedPlatform === 'facebook' && idx === 10 && (
                                        <div className="mt-2 space-y-2">
                                            ...
                                            <div className="flex items-center gap-2 bg-black/30 rounded-lg p-2">
                                                <code className="text-green-400 text-xs flex-1 break-all">aicontents.vip</code>
                                                <button onClick={() => copyToClipboard('aicontents.vip')} className="p-1.5 hover:bg-white/10 rounded-lg"><Copy size={12} className="text-slate-400" /></button>
                                            </div>
                                        </div>
                                    )}
                                    {selectedPlatform === 'facebook' && idx === 11 && (
                                        <div className="mt-2 flex items-center gap-2 bg-black/30 rounded-lg p-2">
                                            <code className="text-green-400 text-xs flex-1 break-all">https://aicontents.vip/privacy-policy</code>
                                            <button onClick={() => copyToClipboard('https://aicontents.vip/privacy-policy')} className="p-1.5 hover:bg-white/10 rounded-lg"><Copy size={12} className="text-slate-400" /></button>
                                        </div>
                                    )}
                                    {selectedPlatform === 'facebook' && idx === 12 && (
                                        <div className="mt-2 flex items-center gap-2 bg-black/30 rounded-lg p-2">
                                            <code className="text-green-400 text-xs flex-1 break-all">https://aicontents.vip/terms-of-service</code>
                                            <button onClick={() => copyToClipboard('https://aicontents.vip/terms-of-service')} className="p-1.5 hover:bg-white/10 rounded-lg"><Copy size={12} className="text-slate-400" /></button>
                                        </div>
                                    )}
                                    {selectedPlatform === 'facebook' && idx === 13 && (
                                        <div className="mt-2 flex items-center gap-2 bg-black/30 rounded-lg p-2">
                                            <code className="text-green-400 text-xs flex-1 break-all">https://aicontents.vip/data-deletion</code>
                                            <button onClick={() => copyToClipboard('https://aicontents.vip/data-deletion')} className="p-1.5 hover:bg-white/10 rounded-lg"><Copy size={12} className="text-slate-400" /></button>
                                        </div>
                                    )}
                                </div>
                            </li>
                    ...
                    <div className="flex gap-2 pt-4 border-t border-white/10">
                        ...
                        <a href={currentPlatform.docsUrl} target="_blank" rel="noopener noreferrer"
                            className="flex-1 flex flex-col items-center justify-center gap-2 px-4 py-3 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/30 rounded-xl transition-all text-purple-400 hover:text-purple-300">
                            <ExternalLink size={20} />
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                    <div className="flex items-center gap-3 mb-6">
                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${currentPlatform.bgGradient} flex items-center justify-center`}>
                            <Icon size={24} className="text-white" />
                        </div>
                        <div>
                            <h2 className="text-xl font-bold text-white">{currentPlatform.name} Credentials</h2>
                            <p className="text-sm text-slate-400">กรอก API Credentials</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {currentPlatform.fields.map((field) => (
                            <div key={field.id}>
                                ...
                                <div className="relative">
                                    <input
                                        type={field.type === 'password' && !showSecrets[`${selectedPlatform}_${field.id}`] ? 'password' : 'text'}
                                        value={appSettings[selectedPlatform]?.[field.id] || ''}
                                        ...
                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-orange-500 pr-12"
                                    />
                                    {field.type === 'password' && (
                                        <button
                                            type="button"
                                            onClick={() => setShowSecrets(prev => ({
                                                ...prev,
                                                [`${selectedPlatform}_${field.id}`]: !prev[`${selectedPlatform}_${field.id}`]
                                            }))}
                                            className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white"
                                        >
                                            {showSecrets[`${selectedPlatform}_${field.id}`] ? <EyeOff size={18} /> : <Eye size={18} />}
                                        </button>
                                    )}
                                </div>
                            </div>
                    ...
                    <button
                        ...
                        className="w-full mt-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-500 hover:to-red-500 text-white font-bold rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                    >
